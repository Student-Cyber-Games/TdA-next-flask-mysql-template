...
1. **Backend dependencies** (uses `uv` for Python package management):
   ```bash
   cd apps/server
   uv sync
   ```
   This command installs all Python dependencies defined in `pyproject.toml`.
...
- **`uv` is not installed**: Install `uv` using `curl -LsSf https://astral.sh/uv/install.sh | sh` (Linux/MacOS) or `powershell -c "irm https://astral.sh/uv/install.ps1 | iex"` (Windows)
...
1. **Backend závislosti** (používá `uv` pro správu Python balíčků):
   ```bash
   cd apps/server
   uv sync
   ```
   Tento příkaz nainstaluje všechny Python závislosti definované v `pyproject.toml`.
...
- **`uv` není nainstalován**: Nainstalujte `uv` pomocí `curl -LsSf https://astral.sh/uv/install.sh | sh` (Linux/MacOS) nebo `powershell -c "irm https://astral.sh/uv/install.ps1 | iex"` (Windows)
...
Jak odevzdat svojí aplikaci můžete najít v našich [Jak nasadit aplikaci na Tour de Cloud](https://tourdeapp.cz/vzdelavaci-materialy/jak-deploy)
